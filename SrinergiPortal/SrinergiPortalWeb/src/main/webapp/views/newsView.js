...
*/window.newsView=Backbone.View.extend({
	initialize:function()
	{ 	
	  this.template = _.template($('#newstemplate').html());
	},
   render:function()
   { 
	   columns = [
...
save:function()
   ...
   { 
       ...
       if($("#roletype").val()=="E")
    	  roleType="External";
    	   ...
    	   roleType="Internal";
	   var searchmodel=new newsSearchModel().set({
		   effectiveFrom:$("#ef1date").val(),
		   effectiveTo:$("#et1date").val(),
		   role: $("#role").val(),
		   ...
		   success:function(response){
				...
				*/	 $("#datagrid").empty();
					 ...
					 var meslist = new Array();
						...
						$('.dropdownwrap').hide();
				     var searchcollection=new newsDetailsCollection(meslist);
   ...
   newcontent:function()
   {
	   ...
	   $("#save").attr('disabled', false);
	   $("#docFile").val("");
	   this.model=new newsDetailsModel();
	   $("#role1").empty();
	   $("#status1").empty();
	   $("#roletype1").empty();
	   $("#save").focus();
	   ...
	   for(var i=0 ;i < roles.length; i++ )
 	   {
          ...
          if(roles[i].roleType==roleTypes[0].roleTypeCode)
		        	{
