...
(function($, undefined) {
	...
	$
			.extend(
					$.ui,
					{
						// $.ui.plugin is deprecated. Use the proxy pattern
						// instead.
						plugin : {
					...
					$.ui.mouse,
					{
						...
						_setContainment : function() {
							...
							if (!(/^(document|window|parent)$/)
								...
								this.containment = [
										(parseInt($(ce).css("borderLeftWidth"),
												10) || 0)
												+ (parseInt($(ce).css(
														"paddingLeft"), 10) || 0),
										(parseInt($(ce).css("borderTopWidth"),
												10) || 0)
												+ (parseInt($(ce).css(
														"paddingTop"), 10) || 0),
							...
							var containment, co, top, left, o = this.options, scroll = this.cssPosition === "absolute"
									&& !(this.scrollParent[0] !== document && $
											.contains(this.scrollParent[0],
													this.offsetParent[0])) ? this.offsetParent
									: this.scrollParent, scrollIsRootNode = (/(html|body)/i)
									.test(scroll[0].tagName), pageX = event.pageX, pageY = event.pageY;
					...
					$.ui.mouse,
					{
						...
						_intersectsWith : function(item) {
							...
							var x1 = this.positionAbs.left, x2 = x1
									...
									+ item.height, dyClick = this.offset.click.top, dxClick = this.offset.click.left, isOverElement = (y1 + dyClick) > t
									&& (y1 + dyClick) < b
									&& (x1 + dxClick) > l
									&& (x1 + dxClick) < r;
							...
							if (!o.placeholder
								...
								o.placeholder = {
									element : function() {
										...
										if (nodeName === "tr") {
											// Use a high colspan to force the
											// td to expand the full
											// width of the table (browsers are
											// smart enough to
...
(function($, undefined) {
	...
	function Datepicker() {
		...
		this._defaults = { // Global defaults for all the date picker instances
			...
			showOptions : {autoSize : true}, // Options for enhanced animations
					...
					Datepicker.prototype,
					{
						...
						_updateDatepicker : function(inst) {
							...
							if (inst === $.datepicker._curInst
									...
									&& inst.input && inst.input.is(":visible")
									&& !inst.input.is(":disabled")
						...
						/*
						 * Retrieve the size of left and top borders for an
						 * element. @param elem (jQuery object) the element of
						 * interest @return (number[2]) the left and top borders
						 */
